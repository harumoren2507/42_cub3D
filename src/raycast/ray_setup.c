...
static void	setup_x_step(t_ray *ray, double player_x)
{
	double	offset;

	ray->delta_x = fabs(1.0 / ray->dir_x);
	offset = player_x - ray->cell_x;
	if (ray->dir_x < 0)
	{
		ray->step_x = -1;
...
static void	setup_y_step(t_ray *ray, double player_y)
{
	double	offset;

	ray->delta_y = fabs(1.0 / ray->dir_y);
	offset = player_y - ray->cell_y;
	if (ray->dir_y < 0)
	{
		ray->step_y = -1;
...
void	ray_setup(t_ray *ray, t_player *player, double ray_angle)
{
	...
	setup_x_step(ray, player->pos_x);
	setup_y_step(ray, player->pos_y);
}
